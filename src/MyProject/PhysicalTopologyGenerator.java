...
//            reqg.createTestTopology_Base(hostSpec, Config.iops,
//                    Config.bw, Config.bw, Config.bw,
//                    Config.latency, Config.fanout, Config.numPods,
//                    Config.redundancy);
//        }
    ...
    protected void createTestTopology(HostSpec hostSpec, long swIops,
        ...
        HostSpec[] h1 = new HostSpec[Config.CoapClient_Number];
        for (int j = 0; j < Config.CoapClient_Number; j++) {
            String hostname = "h1_" + j;
            h1[j] = addHost(hostname, hostSpec);
            System.out.println("Add Host1_ " + j);
            //  addLink(Config.CoapServers.get(pod), h[j], latency);
        }

        //  addCoapServer();
        for (int i = 0; i < Config.CoapClient_Number; i++) {
            addCoapClient("cc_" + i, "core", coreBw, swIops);
            System.out.println("Add coap client " + i);
        }

        for (int i = 0; i < Config.CoapClient_Number; i++) {
            addLink(h1[i], Config.CoapClients.get(i), latency);
            System.out.println("Add link between host1_ " + i + " and coap client " + i);
        }
        ...
        for (int i = 0; i < Config.CoapClient_Number; i++) {
            for (int j = 0; j < redundancy; j++) {
                addLink(Config.CoapClients.get(i), s1[j], latency);
                System.out.println("Add link between coap client " + i + " and switch " + j);
            }
        }
        ...
        for (int pod = 0; pod < Config.CoapServer_Number; pod++) {
            ...
            for (int j = 0; j < fanout; j++) {
                String hostname = "h2_" + pod + "_" + j;
                h[j] = addHost(hostname, hostSpec);
                System.out.println("Add Host2_ " + j);
                addLink(Config.CoapServers.get(pod), h[j], latency);
            }
        }
    ...
    protected void createTestTopology_Base(HostSpec hostSpec, long swIops,
            long coreBw, long aggrBw, long edgeBw,
            double latency, int fanout, int numPods, int redundancy)
            throws SocketException, URISyntaxException, Exception {
        // core, aggregation, edge
        // Core switch       
        ...
        for (int i = 0; i < Config.CoapClient_Number; i++) {
            if (i % 4 == 0) {
                addLink(Config.CoapClients.get(i), s1[0], latency);
                System.out.println("Add link between coap client " + i + " and switch " + 0);
            } else if (i % 4 == 1) {
                addLink(Config.CoapClients.get(i), s1[1], latency);
                System.out.println("Add link between coap client " + i + " and switch " + 1);
            }
        }
        ...
        for (int pod = 0; pod < Config.CoapServer_Number; pod++) {
            ...
            // Add link between aggr - edge
            if (pod % redundancy == 2) {
                addLink(s1[2], Config.CoapServers.get(pod), latency);
                System.out.println("Add link between edge " + pod + " and switch " + 2);
            } else if (pod % redundancy == 3) {
                addLink(s1[3], Config.CoapServers.get(pod), latency);
                System.out.println("Add link between edge " + pod + " and switch " + 3);
            }
    ...
    class CoapServerSpec extends NodeSpec {
        ...
        public CoapServerSpec() throws SocketException, Exception {
            ...
            String[] resource_tuple = {"008", "028", "04", ""};
            int i = 0;
            //while (resource_tuple != null) {
            while (i++ < Config.CoapServer_Number) {
...
//                    }
            ...
            //CoapMessage m = new CoapMessage("x1110xf56321".getBytes());
            CoapMessage m = new CoapMessage();
            //CoapRequest cr = new CoapRequest(m);
            //String resource_uri = cr.getRequestUriPath();//"coap://127.0.0.1/test";
            String[] resource_tuple = null;//flags.getStringTuple("-b", 2, "<format> <value>", null, "resource value in PUT or POST requests; format can be: NULL|TEXT|XML|JSON; value can be ASCII or HEX (0x..)");
            String resource_uri = "coap://coap.me";
            //String[] resource_tuple = {"00248", "02468"};
            int resource_format = resource_tuple != null
                    ? CoapResource.getContentFormatIdentifier(resource_tuple[0]) : -1;
            byte[] resource_value = resource_tuple != null ? (resource_tuple[1].startsWith("0x")
                    ? ByteUtils.hexToBytes(resource_tuple[1]) : resource_tuple[1].getBytes()) : null;

            if (method_name.equalsIgnoreCase(method_name)) { // method (e.g. GET, PUT, etc.)
