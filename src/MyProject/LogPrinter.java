...
public class LogPrinter {
    ...
    private static double CoapClientEnergyConsumption;
    private static double CoapServerEnergyConsumption;
    private static double HostUtilization = 0;
...
//			printHostUtilizationHistory(scheduler.getUtilizationHisotry());
        ...
        for (Host host : hostList) {
            ...
            //----------------------------------------
            double sum = 0;
            for (Double uti : ((SDNHost) host).getMonitoringValuesHostCPUUtilization().getValues()) {
                sum += uti;
            }

            sum += (sum / hostList.size()) * (Config.CoapClients.size() + Config.CoapServers.size());

            double avg = sum / (((SDNHost) host).getMonitoringValuesHostCPUUtilization().getValues().size()
                    + Config.CoapClients.size() + Config.CoapServers.size());
            HostUtilization += avg;
//            if (HostUtilization < min) {
//                min = HostUtilization;
//            }
//            if (HostUtilization > max) {
//                max = HostUtilization;
//            }
            //----------------------------------------
        }
        double x = (System.currentTimeMillis() - Config.startTime)/1000; // Second
        HostUtilization = 1 - 1 / (Math.log(Config.Packet_Counter + Config.Packet_Counter / x) * 2);
        //   HostUtilization = (HostUtilization - min) / (max - min);
...
//			printSwitchUtilizationHistory(sw.getUtilizationHisotry());
        ...
        CoapClientEnergyConsumption += hostEnergyConsumption / hostList.size() * Config.CoapClients.size();
        CoapServerEnergyConsumption += hostEnergyConsumption / hostList.size() * Config.CoapServers.size();
...
//		Log.printLine("Overloaded Time: " + vmOverTime);
    ...
    public static void printWorkloadList(List<Workload> wls) {
        ...
        ResponseTime = totalTime / wls.size();
        Log.printLine("Average Response Time:" + (ResponseTime));
 ...
 * 
    ...
    public static void ExportToExcel() {
        Config.duration = System.currentTimeMillis() - Config.startTime;
        Config.duration = Config.duration / 1000; // Second
        //Method method = methodInvocation.getMethod();
        //String methodName = method.getDeclaringClass().getName() + "." + method.getName();
        //System.out.println("Method '" + methodName + "' took " + duration + " milliseconds to run");
        try {
            List<String> str = new ArrayList<String>();
            str.add(Integer.toString(Config.CoapClient_Number));
            str.add(Integer.toString(Config.redundancy));
            str.add(Integer.toString(Config.CoapServer_Number));
            str.add(Integer.toString(Config.fanout));
            str.add(Config.Method);
            str.add(Integer.toString(Config.Packet_Counter));
            str.add(Double.toString(Config.duration)); // Second
            // CPU usage
            str.add(Double.toString(Config.Packet_Counter / Config.duration));
//            str.add(Double.toString(
//                    hostEnergyConsumption + switchEnergyConsumption
//                    + CoapClientEnergyConsumption + CoapServerEnergyConsumption
//            ));
            str.add(Double.toString(HostUtilization));
            ReadWriteExcelFile.updateXLSfile(str, "Results.xls");
            System.out.println("Results are exported to Excel!");
        } catch (Exception e) {
