...
public class SDNDatacenter extends Datacenter {
			...
			case CloudSimTagsSDN.REQUEST_SUBMIT: 
                    {
                        try {
                            ...
                            Logger.getLogger(SDNDatacenter.class.getName()).log(Level.SEVERE, null, ex);
                        }
                    }
				break;
	...
	private void processRequestSubmit(Request req) throws IOException {
	...
	private void processPacketCompleted(Packet pkt) throws IOException {
		pkt.setPacketFinishTime(CloudSim.clock());
		Request req = pkt.getPayload();
		processNextActivity(req);
	}
	
	private void processNextActivity(Request req) throws IOException {
//		Log.printLine(CloudSim.clock() + ": " + getName() + ": Process next activity: " +req);		
	...
	private void processNextActivityTransmission(Transmission tr) throws IOException {
